...
void CNNLayer<T, Share>::backward(const Share<T> &delta, const Share<T> &forwardInput, int micro_batch_idx) {
    ...
    if(micro_batch_idx == -1) {
        ...
        CHECK_F(forwardInput.size()  == MICRO_BATCH_SIZE * conf.imageHeight * conf.imageWidth * conf.inputFeatures, "the size of forwardInput must be consistent with the number of elements per microbatch");
